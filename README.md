...
**Minneapolis–St. Paul market (msp.html).** Fifth market, built to a tighter spec than the others: **guaranteed-3rd-row full-size SUVs only** (Suburban, Tahoe, Yukon/XL, Expedition/Max, Escalade/ESV, Navigator/L, Armada, QX80, Sequoia — no pickups, no trim-dependent 3-rows) within a **hard 50-mile radius of Minneapolis**, enforced by the distance table in `scripts/process_msp.py` since FB's metro search over-reaches (the Milaca/St Cloud dealer cluster at 60+ mi gets dropped). Same value tables as KC, same cache-first ETL, feeds the combined index with its own Market filter entry.

**Messaged list.** Same mechanics as the pass list, different bucket: hit the **✓** next to Draft DM once you've actually messaged a seller, and the row leaves the main hunt — so the board always shows only untouched prospects. The **Messaged: N** toolbar button flips to a review view of everyone you're waiting on (with ↩ to un-mark, e.g. when a seller never replies and you want the row back in play). Stored in localStorage (`carhunt_messaged`) per browser, survives weekly re-scrapes and republishes. Passed = "not interested"; Messaged = "in play, awaiting reply".
...
**How salvage/rebuilt titles are generated (and how the DM handles them).** An insurer "totals" a vehicle whenever the repair estimate exceeds roughly 75% of its pre-damage value; the state then issues a **salvage** title, and after repair plus a state inspection it becomes **rebuilt**. Crucially, the 75% math doesn't care *what* got damaged: hail on every panel or a theft recovery can brand a title with zero safety impact, while collision or flood damage can mean compromised structure. Since the buyer is fine with cosmetic brands but not safety ones, the **Draft DM for branded rows takes a different shape**: it opens by acknowledging the rebuilt/salvage title up front (no gotcha), then kindly asks what generated it — cosmetic vs structural — seeding the question with whatever cause the seller's own stored description already names (hail, theft, flood, fire, vandalism, collision), and asks for original-damage photos or rebuild documentation. Clean-title rows keep the standard clean-title/lien questions.
